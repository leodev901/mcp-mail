...
async def callback_m365_oauth_delegate(
    ...
    if not code or not state or not company_cd or not user_id:
        return HTMLResponse(
    ...
    logger.info(f"동의한 사용자 '{company_cd}.{user_id}'" )
    ...
    logger.info(f"oauth_callback_expires_in: {expires_in}")
    logger.info(
        f"oauth_callback_now_kst_iso: {datetime.now(timezone(timedelta(hours=9))).isoformat()}"
    )
    logger.info(f"oauth_callback_expires_at: {expires_at}")
    logger.info(f"oauth_callback_expires_at_iso: {expires_at.isoformat()}")
