...
- 관련 코드 경로는 `app/core/token_manager.py`, `app/routes/m365_oauth.py`, `cmn/api/endpoint/oauth.py`, `cmn/utils/jwt_manager.py` 입니다.
...
| `cmn/api/endpoint/oauth.py` | OAuth 콜백, 앱 토큰 발급, 사용자 토큰 조회 API |
| `cmn/api/endpoint/logs_router.py` | Tool 로그 / 외부 API 로그 저장 API |
| `cmn/api/endpoint/utils_router.py` | JWT encode/decode 확인용 유틸 API |
...
- `get_db_session_for_compnay()`와 `get_db_session_for_oauth_state()`는 여전히 `X-Company-Code` 헤더 또는 `state` 값을 기준으로 schema 세션을 엽니다.
...
| `GET` | `/api/oauth/m365/callback` | M365 OAuth 콜백 처리 |
| `GET` | `/api/oauth/user/token` | 사용자 위임 토큰 조회 및 필요 시 갱신 |
| `POST` | `/api/logs/tool` | Tool 실행 로그 저장 |
| `POST` | `/api/logs/api` | 외부 API 호출 로그 저장 |
| `GET` | `/utils/jwt/decode` | Bearer JWT를 해석해 현재 사용자 정보 반환 |
| `POST` | `/utils/jwt/encode` | `User` 스키마를 JWT 문자열로 인코딩 |
...
- `app`은 미들웨어, 보안, Tool, 클라이언트가 나뉘어 있어 MCP 서버 확장과 공통 API 진화를 서로 독립적으로 가져가기 좋습니다.
...
- 다만 `cmn` 내부에서는 이미 라우터, dependency, 서비스, 저장소, 공통 모델, 미들웨어, 예외 처리기를 나눠 두어 과도기 상태에서도 설계 방향은 유지하고 있습니다.
...
현재 이 저장소는 `app = MCP 실행 계층`, `cmn = 인증·세션·로그를 담당하는 공통 API 계층`으로 역할을 분리해 가고 있습니다.
